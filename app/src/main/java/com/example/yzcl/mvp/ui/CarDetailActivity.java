            ...
            protected void onSuccess(Headers headers, JSONObject jsonObject) {
                ...
                if(carMessageBean.isSuccess()){
                    ...
                    if(carMessage.getPledger().getPledger_loc().size()==0){
                        ...
                        if((null==carMessage.getPledger().getPledger_loc().get(0).getProvince()||carMessage.getPledger().getPledger_loc().get(0).getCity()==null||carMessage.getPledger().getPledger_loc().get(0).getDistrict()==null)){
                            //是空或者是""都显示请选择
                            tv_sex.setText("请选择");
                        }else if(carMessage.getPledger().getPledger_loc().get(0).getProvince().length()==0&&carMessage.getPledger().getPledger_loc().get(0).getCity().length()==0&&carMessage.getPledger().getPledger_loc().get(0).getDistrict().length()==0){
                            tv_sex.setText("请选择");
                        }else{
                            String tsex="";
                            if(carMessage.getPledger().getPledger_loc().get(0).getProvince()==null){
                            }else{
                                tsex+=carMessage.getPledger().getPledger_loc().get(0).getProvince();
                            }
                            if(carMessage.getPledger().getPledger_loc().get(0).getCity()==null){
                            }else{
                                tsex+=carMessage.getPledger().getPledger_loc().get(0).getCity();
                            }
                            if(carMessage.getPledger().getPledger_loc().get(0).getDistrict()==null){
                            }else{
                                tsex+=carMessage.getPledger().getPledger_loc().get(0).getDistrict();
                            }
                            tv_sex.setText(tsex);
                        }
...
//                    JSONObject pledgerjsonObject1=new JSONObject();
    ...
    private void showopv(String pinid) {
        ...
        HttpRequest.post(Api.getAddress+"?token="+sp.getString(Constant.Token,""),params,new JsonHttpRequestCallback(){
            ...
            protected void onSuccess(Headers headers, JSONObject jsonObject) {
                super.onSuccess(headers, jsonObject);
                Log.i(TAG+"321", jsonObject.toString());
                CityBean cityBean=JSONObject.parseObject(jsonObject.toString(),CityBean.class);
